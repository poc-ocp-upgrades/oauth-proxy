...
func TestRobotsTxt(t *testing.T) {
	...
	proxy := NewOAuthProxy(opts, func(string) bool {
		return true
	})
...
func TestBasicAuthPassword(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: cookieName, Value: value, Path: "/", Expires: time.Now().Add(time.Duration(24)), HttpOnly: true})
	req.AddCookie(proxy.MakeCSRFCookie(req, "nonce", proxy.CookieExpire, time.Now()))
	rw = httptest.NewRecorder()
	proxy.ServeHTTP(rw, req)
...
type PassAccessTokenTestOptions struct{ PassAccessToken bool }
...
func (pat_test *PassAccessTokenTest) getCallbackEndpoint() (http_code int, cookie string) {
	...
	defer _logClusterCodePath()
	rw := httptest.NewRecorder()
	req, err := http.NewRequest("GET", "/oauth2/callback?code=callback_code&state=nonce:", strings.NewReader(""))
	if err != nil {
		return 0, ""
	}
...
func TestSignInPageIncludesTargetRedirect(t *testing.T) {
	...
	if match == nil {
		t.Fatal("Did not find pattern in body: " + signInRedirectPattern + "\nBody:\n" + body)
	}
	if match[1] != endpoint {
		t.Fatal(`expected redirect to "` + endpoint + `", but was "` + match[1] + `"`)
	}
}
...
type ProcessCookieTestOpts struct{ provider_validate_cookie_response bool }
...
func NewProcessCookieTest(opts ProcessCookieTestOpts) *ProcessCookieTest {
	...
	pc_test.proxy.provider = &TestProvider{ValidToken: opts.provider_validate_cookie_response}
...
func TestAuthOnlyEndpointSetXAuthRequestHeaders(t *testing.T) {
	...
	pc_test.proxy.provider = &TestProvider{ValidToken: true}
	pc_test.validate_user = true
	pc_test.rw = httptest.NewRecorder()
	pc_test.req, _ = http.NewRequest("GET", pc_test.opts.ProxyPrefix+"/auth", nil)
	startSession := &providers.SessionState{User: "oauth_user", Email: "oauth_user@example.com", AccessToken: "oauth_token"}
	pc_test.SaveSession(startSession, time.Now())
...
type SignatureAuthenticator struct{ auth hmacauth.HmacAuth }
...
func NewSignatureTest() *SignatureTest {
	...
	authenticator := &SignatureAuthenticator{}
	upstream := httptest.NewServer(http.HandlerFunc(authenticator.Authenticate))
	upstream_url, _ := url.Parse(upstream.URL)
	...
	return &SignatureTest{opts, upstream, upstream_url.Host, provider, make(http.Header), httptest.NewRecorder(), authenticator}
}
...
func (st *SignatureTest) MakeRequestWithExpectedKey(method, body, key string) {
	...
	proxy := NewOAuthProxy(st.opts, func(email string) bool {
		return true
	})
	...
	state := &providers.SessionState{Email: "mbland@acm.org", AccessToken: "my_access_token"}
	...
	st.authenticator.auth = hmacauth.NewHmacAuth(crypto.SHA1, []byte(key), SignatureHeader, SignatureHeaders)
