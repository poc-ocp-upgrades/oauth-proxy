...
func parseURL(to_parse string, urltype string, msgs []string) (*url.URL, []string) {
	...
	if err != nil {
		return nil, append(msgs, fmt.Sprintf("error parsing %s-url=%q %s", urltype, to_parse, err))
	}
	return parsed, msgs
}
func (o *Options) Validate(p providers.Provider) error {
	...
	for _, u := range o.Upstreams {
		...
		if err != nil {
			msgs = append(msgs, fmt.Sprintf("error parsing upstream=%q %s", upstreamURL, err))
			continue
		}
		if upstreamURL.Path == "" {
			upstreamURL.Path = "/"
	...
	if len(o.CompiledAuthRegex) > 0 {
		...
		if err != nil {
			msgs = append(msgs, fmt.Sprintf("error compiling regex=%q %s", o.ProxyPrefix+"*", err))
		}
	...
	if o.PassAccessToken || (o.CookieRefresh != time.Duration(0)) {
		...
		if valid_cookie_secret_size == false {
			...
			msgs = append(msgs, fmt.Sprintf("cookie_secret must be 16, 24, or 32 bytes "+"to create an AES cipher when "+"pass_access_token == true or "+"cookie_refresh != 0, but is %d bytes.%s", len(secretBytes(o.CookieSecret)), suffix))
		}
	}
	if o.CookieRefresh >= o.CookieExpire {
		msgs = append(msgs, fmt.Sprintf("cookie_refresh (%s) must be less than "+"cookie_expire (%s)", o.CookieRefresh.String(), o.CookieExpire.String()))
	}
...
func parseSignatureKey(o *Options, msgs []string) []string {
	...
	if len(components) != 2 {
		return append(msgs, "invalid signature hash:key spec: "+o.SignatureKey)
	}
