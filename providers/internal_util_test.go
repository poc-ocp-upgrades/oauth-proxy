...
type ValidateSessionStateTestProvider struct{ *ProviderData }
...
func NewValidateSessionStateTest() *ValidateSessionStateTest {
	...
	vt_test.backend = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	vt_test.provider = &ValidateSessionStateTestProvider{ProviderData: &ProviderData{ValidateURL: &url.URL{Scheme: "http", Host: backend_url.Host, Path: "/oauth/tokeninfo"}}}
...
func TestValidateSessionStateValidTokenWithHeaders(t *testing.T) {
	...
	assert.Equal(t, true, validateToken(vt_test.provider, "foobar", vt_test.header))
}
